...
class Pitcher < ActiveRecord::Base
  ...
  def self.get_zips_pitcher_data
    ...
    data.each do |x|
      Pitcher.create(name: x[0].join(","), team: x[1].join(","), wins: x[2].join(","), games: x[6].join(","), gs: x[5].join(","), ip: x[7].join(","), hits: x[8].join(","), er: x[9].join(","), homers: x[10].join(","), so: x[11].join(","), whip: x[13].join(",") )
    end
  end
